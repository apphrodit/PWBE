// POO Criar um Objeto JavaScript
const objeto = {}
objeto.nome = "Márcia"
objeto.idade = 19
objeto.peso = 66.5
objeto.altura = 1.58
//Mostrar Objeto
console.log(objeto)
//Métodos
objeto.getImc = () =>{
    return Number((objeto.peso / (objeto.altura ** 2)).toFixed(2));
}
// Mostrar o resultado do método
console.log(objeto.getImc())
//Incorporar o resultado do Método a um atributo
objeto.imc= objeto.getImc()
//Mostar o objeto com o Método incorporado
console.log(objeto)
//Resultado
/*
{
    nome: 'Márcia',
    idade: 19,
    peso: 66.5,
    altura: 1.58,
    getImc : [Function (anonymus)],
    imc: 26.64
}
*/

//Criara um Objeto JavaScript a partir de uma Classe
class Paciente{
    //Atributos
    nome
    idade
    peso
    altura
    imc = this.getImc()
    //Método
    getImc(){
        return Number((this.peso / this.altura **2).toFixed(2))
    }
}
//Mostar somente a classe
console.log(Paciente)
// Instânciar a classe paciente em um objeto
const paciente1 = new Paciente()
//Mostrar o objeto instanciado na classe Paciente
console.log(paciente1)
// Objeto paciente1 está com todos os atributos e métodos indefinidos
paciente1.nome = 'Alfredo'
paciente1.idade = 25
paciente1.peso = 88
paciente1.altura=  1.92
//Mostrar Paicente 1
console.log(paciente1)
paciente1.imc = paciente1.getImc()
console.log(paciente1.getImc)